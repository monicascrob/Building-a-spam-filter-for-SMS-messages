...
In this project, our goal was to teach the computer how to classify new messages. 
We used the multinomial Naive Bayes algorithm and a dataset of 5,572 SMS messages already classified by humans. 
...
We started by splitting the dataset into 2:
...
We then calculcated the constants used in the Naive Bayes algorithm.
We finally built 2 functions:
 - one to classify the messages
 - another one to measure the spam filter's accuracy
 
Conclusion
In this project we managed to build a spam filter for SMS messages using the Naive Bayes algorithm. 
Our goal to have at least 80% accuracy was overachieved, we managed to obtain over 98% accuracy.
